...
# This program was used to create Fig. 4.2 and the data for 4.1, 4.3 of the manuscript.
# It plots w_i of synapses during an example simulation.
# Furthermore it plots the CV  of w_i of 7 synapses. 
# For combining the results from multiple simulations into one figure (like Fig.4.1, 4.3) please run coefficientOfVariation.py.
...
for i in [6,5,4,3,2,1,0]:
    plt.plot(time, wi[i][:counter],'k',color=color_lines[i],label=labels[i],linewidth=0.3)
...
plt.ylabel(r'$VK \; [{\rm \%}]$', fontsize=12) # in German it is Variationskoeffizient (VK)
