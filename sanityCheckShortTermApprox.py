...
# Short-Term-Approximation (delta, gamma = 0)
...
def show_gillespie(results, number_species, histo_range, name_file):
    ...
    plt.show()
    f.savefig("short_X.pdf", bbox_inches='tight')
...
def show_gillespie_without_p(results, number_species, histo_range, name_file):
    ...
    #plt.title(r'$F=0.5$', fontsize=12)
    plt.show()
    f.savefig("short_without_p.pdf", bbox_inches='tight')
...
def gillespie_algo(number_actual_synapses, init, rates, sub_stoch, prod_stoch, tmax, n_max):
    ...
    average = sum(mol_cv)
    average /= len(mol_cv) # [Durchschnitt1, Durchschnitt2,...]
    ...
    #return(np.array(store_filling_fraction_av), np.array(coefficient_variation))
###################################################
######## Run the main program #####################
###################################################

# define the stochiometry of the substrates and products separatelly
sub_stoch, prod_stoch = reactions_stoch("X4+X5->X1,X4+X6->X2,X4+X7->X3,X1->X4+X5,X2->X4+X6,X3->X4+X7")


# define the initial conditions of the reactants
F = 0.5 # for alpha and p
slots_s1 = 40
slots_s2 = 60
slots_s3 = 80
...
rates = np.array([alpha,alpha,alpha,beta,beta,beta])
init1 = np.array([0,0,0,p,slots_s1,slots_s2,slots_s3])
number_actual_synapses = deepcopy(init1[4:])
slots_ff_percent = FF(sum(number_actual_synapses), init1[3],rates[0],rates[-1])
less_pool = round(init1[4]*slots_ff_percent)+round(init1[5]*slots_ff_percent)+round(init1[6]*slots_ff_percent)
if less_pool > init1[3]:
    number_empty_slots = sum(init1[4:])
    init1[0] = ((init1[4]/number_empty_slots)*init1[3])//1
    init1[1] = ((init1[5]/number_empty_slots)*init1[3])//1
    init1[2] = ((init1[6]/number_empty_slots)*init1[3])//1
else:
    init1[0] = round(init1[4]*slots_ff_percent)
    init1[1] = round(init1[5]*slots_ff_percent)
    init1[2] = round(init1[6]*slots_ff_percent)
init1[3] -= sum(init1[:3])
init1[4] -= init1[0]
init1[5] -= init1[1]
init1[6] -= init1[2]
...
data_in.write("Theoretical Filling Fraction: "+str(np.round(slots_ff_percent,2))+" \n")
...
for r in range(1,sim_round+1):
    ...
    while True:
        ...
        # plot for checking total size of receptors and slots
        store_time = results[0]
...
print("x- and y-axis logarithmically scaled:")
...
number_slots = [40,60,80]
fig, ax = plt.subplots()
ax.plot(number_slots, cv[:3], '.')

plt.xscale('log')
plt.yscale('log')
plt.ylabel(r'$VK \; [{\rm \%}]$', fontsize=12)
plt.xlabel(r'$s_i$', fontsize=12)
plt.show()
fig.savefig(name_sim+" xy_log_scaled.pdf")

print("")
print("x-axis logarithmically scaled:")
...
plt.xlabel(r'$s_i$', fontsize=12)
plt.show()
fig.savefig(name_sim+" x_log_scaled.pdf")

number_slots = ["Synapse 1","Synapse 2","Synapse 3","Pool"]
fig, ax = plt.subplots()
ax.plot(number_slots, cv, '.')

plt.yscale('log')
plt.ylabel(r'$VK \; [{\rm \%}]$', fontsize=12)
plt.show()
fig.savefig(name_sim+" xy_log_scaled_with_p.pdf")
...
plt.ylabel(r'$VK \; [{\rm \%}]$', fontsize=12)
plt.show()
fig.savefig(name_sim+" x_log_scaled_with_p.pdf")
